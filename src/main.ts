...
const MAP_CONFIGS: Record<string, MapConfig> = {
  Bhop_Seal: {
    ...
    ladders: [
      {
        bottomRight: [37.693, 9.81, 6.95],
        bottomLeft: [38.289, 9.847, 7.781],
        topLeft: [38.331, 18.0, 7.834],
        topRight: [37.835, 18.0, 7.213],
        thickness: 0.35,
      },
    ],
    checkpoints: [
      { position: [-27.81, 10.77, 25.52], radius: 1.2 },
      { position: [-31.49, 11.39, 26.5], radius: 1.2 },
      { position: [-34.16, 11.25, 24.15], radius: 1.2 },
      { position: [-33.88, 11.39, 17.55], radius: 1.2 },
      { position: [-23.97, 15.19, 9.12], radius: 1.2 },
      { position: [3.8, 13.26, 7.34], radius: 1.2 },
      { position: [25.76, 13.75, 3.63], radius: 1.2 },
      { position: [37.35, 18.56, 7.57], radius: 1.2 },
      { position: [32.28, 24.69, 12.45], radius: 1.2 },
      { position: [2.73, 24.34, 1.45], radius: 1.2 },
      { position: [-7.43, 28.18, -2.39], radius: 1.2 },
      { position: [-32.44, 18.94, -22.24], radius: 1.2 },
      { position: [-40.93, 19.04, -21.98], radius: 1.2 },
      { position: [-39.29, 23.85, -18.0], radius: 1.2 },
      { position: [-35.68, 30.57, 3.79], radius: 1.2 },
    ],
    skybox: [
  ...
  Bhop_XMas: {
    ...
    chronometer: {
      start: { position: [18.87, 2.302, 14.176], radius: 2 },
      finish: { position: [16.593, 20.706, -33.441], radius: 2 },
    },
    ladders: [],
    lighting: {
      lightDirection: [0.0, 0.03, 0.06],
      lightIntensity: 0.53,
    ...
    checkpoints: [
      ...
      { position: [4.69, 7.07, -0.6], radius: 1.2 },
...
const startGame = async (mapConfig: MapConfig) => {
  try {
    ...
    if (!arcticArmsObj) {
      console.warn(
        `Falha ao carregar o objeto ${selectedSkinConfig.name} arms`
      );
    }

    const spawnPosition: [number, number, number] = [
      ...mapConfig.spawnPosition,
    ];
    camera.setPosition(spawnPosition);

    let deadZoneSystem: DeadZoneSystem;

    if (mapConfig.name === "Bhop XMas") {
      deadZoneSystem = createXmasDeadZones(spawnPosition);
    } else if (mapConfig.name === "Bhop Seal") {
      deadZoneSystem = createSealDeadZones(spawnPosition);
    } else {
      deadZoneSystem = new DeadZoneSystem();
      deadZoneSystem.setSpawnPoint(
        spawnPosition[0],
        spawnPosition[1],
        spawnPosition[2]
      );
    ...
    const forwardSlider = document.getElementById(
      "forward-slider"
    ) as HTMLInputElement;
    const rightSlider = document.getElementById(
      "right-slider"
    ) as HTMLInputElement;
    const upSlider = document.getElementById("up-slider") as HTMLInputElement;
    const forwardValue = document.getElementById(
      "forward-value"
    ) as HTMLElement;
    ...
    const armsControls = document.getElementById(
      "arms-controls"
    ) as HTMLElement;
    const lightingControls = document.getElementById(
      "lighting-controls"
    ) as HTMLElement;
    const postfxControls = document.getElementById(
      "postfx-controls"
    ) as HTMLElement;
    ...
    const fisheyeSlider = document.getElementById(
      "fisheye-slider"
    ) as HTMLInputElement;
    const scanlineSlider = document.getElementById(
      "scanline-slider"
    ) as HTMLInputElement;
    const rgbOffsetSlider = document.getElementById(
      "rgbOffset-slider"
    ) as HTMLInputElement;
    const vignetteSlider = document.getElementById(
      "vignette-slider"
    ) as HTMLInputElement;
    const waveAmpSlider = document.getElementById(
      "waveAmp-slider"
    ) as HTMLInputElement;
    const waveFreqSlider = document.getElementById(
      "waveFreq-slider"
    ) as HTMLInputElement;
    ...
    const fisheyeValue = document.getElementById(
      "fisheye-value"
    ) as HTMLElement;
    const scanlineValue = document.getElementById(
      "scanline-value"
    ) as HTMLElement;
    const rgbOffsetValue = document.getElementById(
      "rgbOffset-value"
    ) as HTMLElement;
    const vignetteValue = document.getElementById(
      "vignette-value"
    ) as HTMLElement;
    const waveAmpValue = document.getElementById(
      "waveAmp-value"
    ) as HTMLElement;
    const waveFreqValue = document.getElementById(
      "waveFreq-value"
    ) as HTMLElement;
    ...
    const lightDirXSlider = document.getElementById(
      "lightDirX-slider"
    ) as HTMLInputElement;
    const lightDirYSlider = document.getElementById(
      "lightDirY-slider"
    ) as HTMLInputElement;
    const lightDirZSlider = document.getElementById(
      "lightDirZ-slider"
    ) as HTMLInputElement;
    const lightIntensitySlider = document.getElementById(
      "lightIntensity-slider"
    ) as HTMLInputElement;
    const ambientSlider = document.getElementById(
      "ambient-slider"
    ) as HTMLInputElement;
    const shininessSlider = document.getElementById(
      "shininess-slider"
    ) as HTMLInputElement;

    const lightDirXValue = document.getElementById(
      "lightDirX-value"
    ) as HTMLElement;
    const lightDirYValue = document.getElementById(
      "lightDirY-value"
    ) as HTMLElement;
    const lightDirZValue = document.getElementById(
      "lightDirZ-value"
    ) as HTMLElement;
    const lightIntensityValue = document.getElementById(
      "lightIntensity-value"
    ) as HTMLElement;
    const ambientValue = document.getElementById(
      "ambient-value"
    ) as HTMLElement;
    const shininessValue = document.getElementById(
      "shininess-value"
    ) as HTMLElement;
    ...
    const syncLighting = () => {
      ...
      renderer.setLightIntensity(
        parseFloat(lightIntensitySlider?.value ?? "1")
      );
      ...
      if (lightIntensityValue)
        lightIntensityValue.textContent = (
          lightIntensitySlider ? parseFloat(lightIntensitySlider.value) : 1
        ).toFixed(2);
      if (ambientValue)
        ambientValue.textContent = (
          ambientSlider ? parseFloat(ambientSlider.value) : 0.2
        ).toFixed(2);
      if (shininessValue)
        shininessValue.textContent = shininessSlider
    ...
    if (mapConfig.lighting) {
      ...
      if (lightDirXSlider)
        lightDirXSlider.value = l.lightDirection[0].toString();
      if (lightDirYSlider)
        lightDirYSlider.value = l.lightDirection[1].toString();
      if (lightDirZSlider)
        lightDirZSlider.value = l.lightDirection[2].toString();
      if (lightIntensitySlider)
        lightIntensitySlider.value = l.lightIntensity.toString();
    ...
    const updateArcticArmsTransform = (dt: number = 0) => {
      ...
      const finalPos = vec3.add(
        vec3.add(vec3.add(anchor, offsetForward), offsetRight),
        offsetUp
      );
    ...
    scene.start((scene, deltaTime) => {
      ...
      if (input.isKeyPressed("KeyP")) {
        if (!pickKeyPressed) {
          ...
          if (hit) {
            const p = hit.point;
            console.log(
              `Look hit: (${p[0].toFixed(3)}, ${p[1].toFixed(
                3
              )}, ${p[2].toFixed(3)}) dist=${hit.distance.toFixed(2)}`
            );
          } else {
      ...
      if (noclip) {
        ...
        if (input.isKeyPressed("KeyW"))
          camera.move("FORWARD", noclipSpeed * deltaTime);
        if (input.isKeyPressed("KeyS"))
          camera.move("BACKWARD", noclipSpeed * deltaTime);
        if (input.isKeyPressed("KeyA"))
          camera.move("LEFT", noclipSpeed * deltaTime);
        if (input.isKeyPressed("KeyD"))
          camera.move("RIGHT", noclipSpeed * deltaTime);
        if (input.isKeyPressed("Space"))
          camera.move("UP", noclipSpeed * deltaTime);
        if (
          input.isKeyPressed("ControlLeft") ||
          input.isKeyPressed("ControlRight")
        )
          camera.move("DOWN", noclipSpeed * deltaTime);
      } else {
        // modo normal
        playerMovement.updateDirections(camera.getFront(), camera.getRight());

        const onLadder = ladderSystem.update(
          deltaTime,
          playerMovement,
          input,
          collision
        ...
        if (deadZoneSystem.checkPlayerInDeadZone(playerPos)) {
          const target = chronometer.isRunning()
            ? checkpointSystem.getLastCheckpoint()
            : deadZoneSystem.getSpawnPoint();
        ...
        if (chronoRunning) {
          checkpointSystem.update(
            playerMovement.getEyePosition() as [number, number, number]
          );
        }
        ...
        if (groundedElement) {
          groundedElement.textContent = `Grounded: ${
            playerMovement.getIsGrounded() ? "Yes" : "No"
          }`;
        }
      }

      const eyePos = playerMovement.getEyePosition() as [
        number,
        number,
        number
      ];
