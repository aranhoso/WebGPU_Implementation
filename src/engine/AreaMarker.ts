...
export class AreaMarker {
    ...
    public update(input: Input, getPosition: () => [number, number, number]): void {
        if (input.isKeyPressed('ArrowLeft')) {
            if (!this.leftArrowPressed) {
                this.points = [];
                this.isActive = true;
                console.log('%c[Area Marking] Started new area. Press ↑ to add points.', 'color: #00ff00; font-weight: bold');
        ...
        if (input.isKeyPressed('ArrowUp')) {
            if (!this.upArrowPressed && this.isActive) {
                ...
                console.log(`%c[Area Marking] Point ${this.points.length} added: [${pos[0].toFixed(3)}, ${pos[1].toFixed(3)}, ${pos[2].toFixed(3)}]`, 'color: #00ffff');
        ...
        if (input.isKeyPressed('ArrowDown')) {
            if (!this.downArrowPressed && this.isActive && this.points.length > 0) {
                ...
                console.log(`%c[Area Marking] Removed point: [${removed![0].toFixed(3)}, ${removed![1].toFixed(3)}, ${removed![2].toFixed(3)}]. ${this.points.length} points remaining.`, 'color: #ffaa00');
    ...
    private exportArea(): void {
        this.isActive = false;
        console.log('%c[Area Marking] Area completed! Exporting coordinates:', 'color: #00ff00; font-weight: bold');
        ...
        this.points.forEach((p, i) => {
            console.log(`  [${p[0].toFixed(3)}, ${p[1].toFixed(3)}, ${p[2].toFixed(3)}]${i < this.points.length - 1 ? ',' : ''} // Point ${i + 1}`);
        });
    }
