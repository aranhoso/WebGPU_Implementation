...
export class CollisionSystem {
  ...
  public loadMeshCollision(mesh: Mesh): void {
    ...
    console.log(
      `CollisionSystem: Carregados ${this.triangles.length} triângulos para colisão (grid cells: ${this.spatialGrid.size})`
    );
  }
      ...
      for (const tri of nearbyTris) {
        ...
        if (collision.collided) {
          ...
          if (isWalkable) {
            ...
            const horizontalLen = Math.sqrt(
              pushNormal[0] * pushNormal[0] + pushNormal[2] * pushNormal[2]
            );

            if (horizontalLen > 0.001) {
              const horizontalNormal = [
                pushNormal[0] / horizontalLen,
                0,
                pushNormal[2] / horizontalLen,
              ];
              const pushVector = vec3.scale(
                horizontalNormal,
                collision.depth + 0.01
              );
              resultPos[0] += pushVector[0];
              resultPos[2] += pushVector[2];
            }
  ...
  private sphereTriangleCollision(
    ...
    if (distSq < radiusSq) {
      ...
      const normal =
        dist > 0.0001 ? vec3.scale(diff, 1 / dist) : triangle.normal;
  ...
  private closestPointOnTriangle(
    ...
    return vec3.add(
      v0,
      vec3.add(vec3.scale(ab, vCoord), vec3.scale(ac, wCoord))
    );
  }
  ...
  public getGroundInfo(
    position: number[]
  ): { height: number; normal: number[]; isWalkable: boolean } | null {
    const rayDir = [0, -1, 0];
    let closestHit: {
      height: number;
      normal: number[];
  ...
  ): { point: number[]; distance: number; normal: number[] } | null {
