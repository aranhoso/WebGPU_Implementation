...
export class Chronometer {
  ...
  private ensureMarkers(show: boolean) {
    ...
    if (show && !this.markersShown) {
      ...
      const finishObj: SceneObject = {
        ...
        modelMatrix: mat4.translate(
          mat4.identity(),
          this.finish.position[0],
          this.finish.position[1],
          this.finish.position[2]
        ),
      };
  ...
  private buildMarkerMesh(scale: number): Mesh {
    ...
    const indices = [
      0, 1, 2, 0, 2, 3, 4, 6, 5, 4, 7, 6, 8, 9, 10, 8, 10, 11, 12, 14, 13, 12,
      15, 14, 16, 17, 18, 16, 18, 19, 20, 22, 21, 20, 23, 22,
    ];
  ...
  private render(now: number) {
    ...
    lines.push(
      `Chrono: ${
        this.running ? this.formatSeconds(current) + " (running)" : "idle"
      }`
    );
