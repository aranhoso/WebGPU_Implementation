...
fn sky_vert_main(@builtin(vertex_index) VertexIndex : u32) -> VertexOutput {
    ...
    output.Position = vec4<f32>(positions[VertexIndex], 1.0, 1.0); // z = 1 -> posiciona atrás da cena (lembrar do frustum e o pipeline)
    var x: f32 = positions[VertexIndex].x;
    var y: f32 = positions[VertexIndex].y;

    // calcula a direção do raio com base no x e y da camera
